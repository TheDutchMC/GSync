...
pub fn sync(config: &Configuration, env: &Env) -> Result<()> {
    ...
    for child in children {
        ...
        sync_child(child, env, true, &access_token)?;
    }
...
fn sync_child(child: Child, env: &Env, at_root: bool, access_token: &str) -> Result<()> {
    match child {
        Child::Directory(dir) => {
            ...
            match record {
                ...
                None => {
                    ...
                    let id = drive::create_folder(access_token, &dir.name, &parent_id)?;
            ...
            for child in dir.children {
                sync_child(child, env, false, &access_token)?;
            }
        },
        Child::File(path) => {
            ...
            match record {
                Some((id, mod_time)) => {
                    ...
                    if has_changed {
                        ...
                        drive::update_file(access_token, &path, &id)?;
                    }
                ...
                None => {
                    let parent_id = if at_root {
                        ...
                        //Parent is always Some, because we've had to traverse it to get to the child.
                        let (id, _) = get_file_record(path.parent().unwrap(), env)?.unwrap();
                        id
                    };

                    println!("Info: Uploading file '{}'", &path.file_name().unwrap().to_str().unwrap());
                    let id = drive::upload_file(access_token, &path, &parent_id)?;
...
fn remote_delete_removed(env: &Env) -> Result<()> {
    ...
    while let Ok(Some(row)) = result.next() {
        ...
        drive::delete_file(&access_token, &id)?;
    }
